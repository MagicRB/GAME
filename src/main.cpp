...
#else
typedef void (*f_funci)(databank*);
typedef void (*f_funco)(databank*, std::vector<std::string>);
#endif // _WIN32
    ...
    while (!infile.eof()) // To get you all the lines.
    {
        ...
        for (unsigned i = 0; i < db->svdll.size(); i++)
        {
            if (db->svdll.at(i) == db->vstr(Obj_V, 0))
            {
                ...
                funci(db, Obj_V);
                #else
                f_funco funci = (f_funco)dlsym(db->hvdll.at(i), "loadObjects");
                funci(db, Obj_V);
                #endif // _WIN32
            }
        }
...
void loadMods(databank* db)
{
    ...
    while (std::getline(file, str))
    {
        ...
        funci(db);
        #else
        db->hvdll.push_back(dlopen(("mods/" + str + ".so").c_str(), RTLD_LAZY));
        db->svdll.push_back(str);
        if (db->hvdll.at(db->hvdll.size() - 1) == NULL)
        {
            std::cout << dlerror() << std::endl;
        }
        f_funci funci = (f_funci)dlsym(db->hvdll.at(db->hvdll.size() - 1), "initMod");
        funci(db);
        #endif // _WIN32
    }
}
